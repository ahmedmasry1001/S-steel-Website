...
const Contact = () => {
  ...
  // Load contact cards and company information
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch contact cards
        const cardsResponse = await fetch('/api/contact-cards');
        if (cardsResponse.ok) {
          const cardsData = await cardsResponse.json();
          
          // Transform API data to match expected format with gradients
          const gradients = [
          ...
          const transformedCards = cardsData.map((card, index) => ({
            id: card.id,
            title: card.title,
            details: card.details,
            subDetails: card.subDetails,
            emoji: card.emoji,
          ...
          setContactCards(getDefaultContactInfo());
        }

        // Fetch company information
        const companyResponse = await fetch('/api/home-content');
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full">
        ...
        <span className="ml-2 text-text-secondary">Loading contact information...</span>
      </div>
    );
  }
      ...
      <div className="relative bg-gradient-to-br from-indigo-50 via-white to-cyan-50 rounded-2xl p-6 border border-gray-100 shadow-xl">
        ...
        <div className="relative">
          ...
          <p className="text-gray-600 text-sm mb-4 max-w-2xl">
            {companyInfo?.companyDescription || 'Ready to start your steel construction project? Contact our expert team for quotes, consultations, or any questions.'}
          </p>
        ...
        <div className="col-span-2 flex flex-col space-y-3">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <div className="bg-gray-100 text-gray-600 px-2 py-1 rounded-lg text-xs font-medium">
              {contactCards.length} ways to reach us
            </div>
          </div>
