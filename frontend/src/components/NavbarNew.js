  ...
  return (
    <nav className="bg-white shadow-lg sticky top-0 z-50 border-b-2 border-industrial-orange">
      <div className="container">
        <div className="flex justify-between items-center py-4">
          ...
          <div className="md:hidden">
            <button
              ...
              className="text-steel-gray hover:text-industrial-orange focus:outline-none focus:text-industrial-orange transition-colors"
            >
              {isOpen ? (
        ...
        {isOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-bg-primary rounded-b-lg shadow-lg">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  to={item.href}
              ...
              <Link
                to="/contact"
                className="block w-full text-center mt-4 btn btn-primary bg-gradient-to-r from-industrial-orange to-industrial-orange-light"
                onClick={() => setIsOpen(false)}
              >
                Get Quote
              </Link>
